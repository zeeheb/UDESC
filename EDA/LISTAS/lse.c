...
void ordenaLista (int count, int entradaOrd[], NOH_tipo_LSE **L, NOH_tipo_LSE **LC);
...
for(i=0;i<10;i++) {
  ...
  if(count>1) {
  for(i=0;i<count-1;i++) {
    ins_fim_lista_2(&(entrada[i]), &L);
  }

  ordenaLista(count, entrada, &L, &LC);
...
void ordenaLista (int count, int entradaOrd[], NOH_tipo_LSE **L, NOH_tipo_LSE **LC) {
  ...
  for(i=0;i<count-1;i++){
    for(int j=0;j<count-1;j++){
        if(entradaOrd[i] < entradaOrd[j]) {
          aux = entradaOrd[i];
          entradaOrd[i] = entradaOrd[j];
          entradaOrd[j] = aux;
        }
    }
  }
    for(i=0;i<count-1;i++){
  ins_fim_lista_2(&entradaOrd[i], LC);
}
